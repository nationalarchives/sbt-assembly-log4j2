...
lazy val root: Project = project
  ...
  .settings(
    ...
    scmInfo := Some(
      ScmInfo(
        url("https://github.com/nationalarchives/sbt-assembly-log4j2"),
        "git@github.com:nationalarchives/sbt-assembly-log4j2.git"
      )
    ),
    developers := List(
      Developer(
        ...
        url = url("https://github.com/nationalarchives/sbt-assembly-log4j2")
      )
    ),
    description := "An sbt plugin used to merge log4j dat files",
    licenses := List("MIT" -> new URL("https://choosealicense.com/licenses/mit/")),
    homepage := Some(url("https://github.com/nationalarchives/sbt-assembly-log4j2")),
    console / initialCommands := """import uk.gov.nationalarchives.sbt._""",
    scriptedLaunchOpts ++= Seq("-Xmx1024M", "-Dplugin.version=" + (ThisBuild / version).value)
